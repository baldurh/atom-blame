...
export default class EditorHandler {
  ...
  subscribeToRepository() {
    this.repository = findRepo(this.editor.getPath());
    if (this.repository) {
  ...
  async getBlameData() {
    if (!this.repository || !this.editor) { return null; }

    const filePath = this.editor.getPath();
    if (!filePath) { return null; }

    const result = await blameFile(filePath);
  ...
  openCommitInBrowser() {
    ...
    const link = getCommitLink(this.editor.getPath(), data.hash, this.repository.getOriginURL());
    if (link) {
      open(link);
    } else {
