...
export default class EditorHandler {
  ...
  openCommitInBrowser() {
    ...
    if (!url) {
      this.blameView.addNotificationTooltip('No origin remote found. Shift-click to copy hash.', 2000);
      return;
    }
    const link = getCommitLink(this.path, data.hash, url);
    if (link) {
      open(link);
    } else {
  ...
  getRemoteUrl() {
    return this.repository && this.repository.getOriginURL();
  }
