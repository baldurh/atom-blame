...
import { isCommitted, findRepo, blame, getCommit, getCommitLink } from './utils';
...
class BlameStatusBarView extends HTMLElement {
  ...
  async registerTooltip(msg) {
    this.disposeTooltip();
    if (msg && this.hash && isCommitted(this.hash)) {
      this.tooltip = await this.addTooltip(msg);
    }
  }
  ...
  async onDidChangeCursorPosition({ row, newBufferPosition = {} }) {
  ...
  async getBlameData() {
    ...
    const result = await blame(this.editor.getPath());
