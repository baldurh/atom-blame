/** @babel */

import Git from 'git-wrapper';
import utils from './';
import configs from '../config/provider';

function parseRemote(remote, config) {
  for (const exp of config.exps) { // eslint-disable-line
    const m = remote.match(exp);
    if (m) {
      return { protocol: m[1], host: m[2], user: m[3], repo: m[4] };
    }
  }
  return null;
}

function buildLink(remote, hash, config) {
  const data = parseRemote(remote, config);
  if (data) {
    return config.template
      .replace('{protocol}', data.protocol || 'https')
      .replace('{host}', data.host)
      .replace('{user}', data.user)
      .replace('{repo}', data.repo)
      .replace('{hash}', hash);
  }
  return null;
}

function getConfig(git, key) {
  return new Promise((resolve, reject) => {
    git.exec('config', { get: true }, [key], (error, result) => {
      if (error) { reject(error); }
      resolve(result);
    });
  });
}

async function getCommitLink(file, hash) {
  const repoPath = utils.findRepo(file);
  if (!repoPath) {
    return null;
  }

  const git = new Git({ 'git-dir': repoPath });

  try {
    const remote = await getConfig(git, 'remote.origin.url');
    const cleanRemote = remote.replace(/(^\s+|\s+$)/g, '');

    for (const config of configs) { // eslint-disable-line
      const link = buildLink(cleanRemote, hash, config);
      if (link) {
        return link;
      }
    }
  } catch (e) {
    console.error(e); // eslint-disable-line
  }
  return null;
}

export default getCommitLink;
